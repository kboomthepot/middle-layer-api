...
async function processJobDemographics(jobId) {
  ...
  try {
    ...
    console.log(
      `ℹ️ [DEMOS] Job ${jobId} location = "${location}", paidAdsStatus = "${paidAdsStatus}"`
    );
    ...
    if (existingRows.length) {
      console.log(
        `ℹ️ [DEMOS] jobs_demographics row already exists for job ${jobId}, updating to pending.`
      );
  ...
  await bigquery.query({
    ...
    // 2c) Update demographicsStatus in jobs table to pending
    console.log(
      '➡️ [DEMOS] Step 2c: Update demographicsStatus = pending in client_audits_jobs'
    );
    await bigquery.query({
      query: `
    ...
    console.log(
      '➡️ [DEMOS] Step 3: Fetch demographics from Client_audits_data.1_demographics'
    );
    ...
    console.log(
      '➡️ [DEMOS] Step 4: Update jobs_demographics with final values'
    );
    await bigquery.query({
      query: `
        UPDATE \`${PROJECT_ID}.${JOBS_DATASET_ID}.${JOBS_DEMOGRAPHICS_TABLE_ID}\`
    ...
    console.log(
      `✅ [DEMOS] Copied demographics into jobs_demographics for job ${jobId}`
    );
    ...
    console.log(
      `✅ [DEMOS] Updated job ${jobId} statuses after demographics processing`
    );
...
app.post('/', async (req, res) => {
  try {
    ...
    console.log(
      `ℹ️ [DEMOS] Demographics processing summary for ${jobId}:`,
      result
    );
